...
public:
    ...
    void free_memory();
    void init(int *ls_local_opt, int *ls_global_opt, lit *ls_unit_clause, int ls_unit_clause_count, const int *ls_clause_lit_count, int ls_solu_feasible, bool ls_memo_type, int ls_h_inc);
...
Decimation::Decimation(lit **ls_var_lit, int *ls_var_lit_count, lit **ls_clause_lit, long long *ls_org_clause_weight, long long ls_top_clause_weight, long long ls_ave_soft_weight)
{
    ...
    ave_soft_weight = ls_ave_soft_weight;
}
        ...
        for (int i = 0; i < var_lit_count[v]; ++i)
        {
            ...
            if (org_clause_weight[c] == top_clause_weight)
            {
                if (var_lit[v][i].sense == 1) {
                    h_true_score[v] += (1 + basis_clause_weight[c]);
                }
                else {
                    h_false_score[v] += (1 + basis_clause_weight[c]);
                }
            }
            else
            {
...
void Decimation::hunit_propagation()
{
    ...
    //c = hunit_clause_queue[hunit_beg_pointer].clause_num;
    ...
    if (sense_in_hunit_clause_queue[v] == -2)
    {
        if (sscore[v] > 0)
        {
            ...
            //sense = local_opt[v];
            sense = memo_type ? local_opt[v] : global_opt[v];
            //sense = rand() % 2;
        }

        for (int i = 0; i < var_lit_count[v]; ++i) {
            c = var_lit[v][i].clause_num;
            basis_clause_weight[c] += 1;  //satlg
            //basis_clause_weight[c] += ave_soft_weight;
        }
    }
    assign(v, sense);
}
...
void Decimation::random_propagation() {
    ...
    for (int i=0; i<min(unassigned_var_count, ht); i++) {
        ...
        if (hscore[v] > 0) {   // hscore == 0 说明是当前情况下的 soft var；hscore 不可能小于0
            break;
        }
    }
