    ...
    for (int c = 0; c < num_clauses; c++)
    {
        ...
        if (org_clause_weight[c] == top_clause_weight) {
            if (best_soln_feasible == 0)
                clause_weight[c] = 1 + basis_clause_weight[c];
            else
                clause_weight[c] = 1;
        }
        else
    ...
    for (tries = 1; tries < max_tries; ++tries)
    {
        ...
        for (step = 1; step < max_flips; ++step)
        {
            ...
            if (step % 1000 == 0 || opt_unsat_weight == 0) {
                ...
                if (elapse_time >= cutoff_time && print60 == 0) {
                    memo_from_type = true;
                    print60 = 1;
        ...
        for (int c = 0; c < num_clauses; ++c) {
            //if (sat_count[c] > 0) basis_clause_weight[c] += 1;  satlg
            if (sat_count[c] > 0) basis_clause_weight[c] += 1;  //ave_soft_weight;
        }
    }
}
